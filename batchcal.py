...
def calculate_Ea_and_save_to_csv(folder_path, output_csv):
    ...
    for file_name in os.listdir(folder_path):
        if file_name.endswith(".cif"):
            # 对每个 CIF 文件进行 Ea 计算
            ea_value = calculate_Ea(os.path.join(folder_path, file_name))
...
def extract_ea_elements_from_csv(csv_filename):
    ...
    with open(csv_filename, "r") as file:
        ...
        data = list(reader)
        for row in data:
            ea_list = eval(row["Ea Value"])  # 将字符串转换为列表
            if len(ea_list) >= 3:  # 确保"Ea"列中有足够的元素
    ...
    plt.title("1000种不同排列结构的BVSE能垒统计")
    plt.xlabel("BVSE 1D energy barrier(eV)")
    plt.ylabel("BVSE 2D energy barrier(eV)")
...
if __name__ == "__main__":
    extract_ea_elements_from_csv("output1000.csv")
    # # 使用示例
    # folder_path = (
    #     r"D:\warehouses\materials_related_program\supercell-windows\LiLaTaCl500"
    # )
